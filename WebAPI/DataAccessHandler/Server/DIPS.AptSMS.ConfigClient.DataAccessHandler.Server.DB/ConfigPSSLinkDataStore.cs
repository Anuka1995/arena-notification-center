...
{
    public class ConfigPSSLinkDataStore : BaseStore, IConfigPSSLinkDataStore
    {
        private static readonly ILog s_log = LogProvider.GetLogger(typeof(ConfigPSSLinkDataStore));

        private readonly DbProviderFactory m_providerFactory;

        public ConfigPSSLinkDataStore(DbProviderFactory providerFactory)
        {
        ...
        public List<SmsConfigurationDTO> GetPSSLinkByHospital(long hospitalId)
        {
            ...
            try
            {
                using (var connection = m_providerFactory.CreateConnection())
                {
                    var oracleparams = ConfigPSSLinkQueryParamsHelper.GetPSSLinkSearchQueryParams(hospitalId);
                    ...
                    foreach (var result in results)
                    {
                        ...
                        smsConfigurationDtos.Add(mapToSmsConfigurationDto(resultDictionary));
                    }
        ...
        private SmsConfigurationDTO mapToSmsConfigurationDto(IDictionary<string, object> dictionary)
        {
